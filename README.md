...
- **0.6.0（当前）** 固定于 main Commit `55a6220`（`git checkout 55a6220` 可复现 npm `dsh-rule-engine@0.6.0` 与 GitHub Release v0.6.0 同源代码——0.6.0 = 本机集成层（localIntegrations 四键）+ 本机痕迹消号；词表文件 `scripts/local-residue-markers.txt` 为本机门禁工具，不入库、不进发布物（见 .gitignore / package.json files 排除）。`55a6220` 之后 `07c7b30`/`677e482` 为固定源文本收尾提交，功能源码以 `55a6220` 为准）。
