...
- **措施类型（kind）**：声明/配置写 kind 名（`approval`/`backup`/`inline-command`…）即绑定对应执行器；历史内部名（`rule12a-approval` 等）经兼容别名表归一为同一 kind，**旧配置与旧规则正文无需改动**；
...
| 版本 | 日期 | 要点 |
| **0.6.5** | 2026-09-20 | chore(comments): 注释里的行号引用改稳定标识（9 处） |
|---|---|---|
...
- **0.6.3** 固定于 main Commit `3e87f9d`（`git checkout 3e87f9d` 可复现 npm `dsh-rule-engine@0.6.3` 与 GitHub Release v0.6.3 同源代码——0.6.3 = 分层残留闸 `dualtrack-check`（判据 A：中文≠个人化）+ 词表全量配置化（lexicons / patterns / criticismPersonal / dualtrack 走 `rule-engine.json`）+ 第三批第 1 批文案层 + 三项门禁修复（`--init` 覆盖保护 / loader-smoke 中文夹具 / 中文目录顿号兼容）。
