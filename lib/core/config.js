...
export const DEFAULT_CONFIG = {
  ...
  correctInject: true,
  selfProtect: true,
  // 未归类工具首调策略（默认口径）："off" 放行并留审计、"ask" 询问、"deny" 拒绝；
  // 缺键等同 "off"；仅字符串取值参与匹配（先去首尾空格再转小写），其它类型与认不出的取值按 "deny" 处理。
  unknownPolicy: "off",
  injectLimitPerRulePerSession: 3,
  taskContractEnabled: false,
  askEnabled: false,
  taskContractMode: "observe",
  // 0.5.15：回合末裁决卡片开关（默认关——面向大众；设置页开关与任务契约同组）
  ...
  handlerDefaultMap: {},
  // §1.3（2026-09-08 第二批）：本机辱骂词表（个人话术，发布面不含）——空=不启用；
  // 通用层只保留语言无关形态检测（连续问叹号/全大写比率），语义判定归 LLM。
  criticismPersonal: []
};
