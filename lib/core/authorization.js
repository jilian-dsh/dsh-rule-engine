...
const TYPE_HINTS = [
  { re: /remove|delete/i, type: "delete" },
  { re: /edit|write|replace/i, type: "write" },
  { re: /backup/i, type: "backup" },
  { re: /git\s+(push|commit)/i, type: "git" },
  ...
  { re: /run|execute|pwsh|bash|node\s+(?!-e\b|-p\b|--eval\b|--print\b)(?:['"])?[\w./\\-]+(?:['"])?(?:\s|$)|(?:npm|pnpm|yarn|bun)\s+(?:run|exec|install|add)|(?:^|[^a-z])(?:tsc|tsdown|vite|webpack|rollup)\b/i, type: "command" },
  ...
  { re: /skill\s+(?:tools?|dir|enable|disable|list|install)\b|\/guard\s+skills?\b/i, type: "skill" },
  // 0.5.12（F1 重证）：network 词表由裸子串改词边界+上下文——`fetch` 出现在目录名/标识符中
  //（dsh-web-fetch-playwright）不再误判；真网络形态（URL/调用/下载动词）仍命中
  { re: /https?:\/\/\S*|(?:^|[^a-z])(?:curl|fetch|wget|iwr|Invoke-WebRequest|Invoke-RestMethod)\s*(?:\(|\.|[^\w]|$)/i, type: "network" },
  ...
  { re: /\b(?:review|read|inspect|check|audit|verify|analyz(?:e|is)?|research|compare|display|open|diagnos(?:e|is)?)\b/i, type: "analysis" },
  // C4（2026-09-04）：archive 独立类型（解压/归档高频用例；物理确认最小范围）
  { re: /unzip|expand-archive|extract-archive|tar\s+-x|7z\s+x/i, type: "archive" }
];

// ── C4（2026-09-04）：类型提示配置化 + 物理确认类型枚举 ──
// typeHintsOverride = null → 内置表；非空 → 默认与内置合并（[...TYPE_HINTS, ...配置]）；
// 空 → 还原内置；{ clear: true } → 仅配置（不含内置）。
 ...
 */
// ── 域 2 第四枪（2026-09-22）：两个标记正则迁配置层（rule-engine.json 的
// delegationMarker／sessionWide 键）。内置＝语言无关最小集；取词走 getter，禁止顶层缓存实例。──
const DELEGATION_MARKER_BUILTIN =
  /\b(?:task|only\s+allow(?:ed)?|limited\s+to|under\s+(?:the\s+)?director(?:y|ies)|execute\s+(?:the\s+)?following|steps?)\b/.source;
const SESSION_WIDE_BUILTIN =
  /\b(?:this\s+session|current\s+session|entire\s+session|whole\s+session|entire\s+profile|whole\s+profile|entire\s+workspace|whole\s+workspace|all\s+subsequent|all\s+remaining|all\s+operations)\b/.source;

let delegationMarkerOverride = null; // 合并后 source | null
let sessionWideOverride = null;

/** 域 2 第四枪：委托框架词；空＝还原内置；非空＝默认合并；非法正则跳过且不覆盖现状 */
export function setDelegationMarker(source, opts = {}) {
  if (typeof source !== "string" || source.length === 0) {
    delegationMarkerOverride = null;
    return;
  }
  try {
    new RegExp(source, "i");
  ...
  delegationMarkerOverride = opts?.clear ? source : `(?:${DELEGATION_MARKER_BUILTIN})|(?:${source})`;
}

/** 域 2 第四枪：会话级范围词；语义同上 */
export function setSessionWide(source, opts = {}) {
  if (typeof source !== "string" || source.length === 0) {
    sessionWideOverride = null;
    return;
  }
  try {
    new RegExp(source, "i");
  ...
  sessionWideOverride = opts?.clear ? source : `(?:${SESSION_WIDE_BUILTIN})|(?:${source})`;
}

function currentDelegationMarker() {
  return new RegExp(delegationMarkerOverride ?? DELEGATION_MARKER_BUILTIN, "i");
}

function currentSessionWide() {
  return new RegExp(sessionWideOverride ?? SESSION_WIDE_BUILTIN, "i");
}
...
export function isDelegationText(text) {
  ...
  return currentDelegationMarker().test(s);
}
...
// 域 2 第三枪（2026-09-22）：六条与 verbRe 迁配置层（rule-engine.json 的 verbTypes／verbRe 键）。
// 内置＝语言无关最小集；取词一律走 getter，禁止顶层缓存实例（配置注入后必须立即生效）。
const VERB_TYPES = [
  { re: /\b(?:delete|remove|clear)\b/i, type: "delete" },
  { re: /\bbackup\b/i, type: "backup" },
  { re: /\b(?:commit|push)\b/i, type: "git" },
  { re: /\bdownload\b/i, type: "network" },
  { re: /\b(?:execute|run)\b/i, type: "any" },
  { re: /\b(?:write|edit|replace)\b/i, type: "write" }
];
const VERB_RE_BUILTIN = /\b(?:delete|remove|clear|edit|replace|write|backup|commit|push|download|execute|run|read|display|open)\b/.source;

let verbTypesOverride = null; // 编译表 | null
let verbReOverride = null;    // 合并后 source | null

function isValidVerbHint(h) {
  if (!h || typeof h.type !== "string" || h.type.length === 0) return false; // 允许 any（与 setTypeHints 相反）
  if (typeof h.re !== "string" || h.re.length === 0) return false;
  try {
    new RegExp(h.re, "i");
    return true;
  } catch {
...
function normalizeVerbHints(list) {
  const out = [];
  for (const h of list) {
    if (!isValidVerbHint(h)) continue;
    // 按 type 去重只留第一条；编 i 不加 g（.test() 无 lastIndex 状态）
    if (!out.some((x) => x.type === h.type)) out.push({ type: h.type, re: new RegExp(h.re, "i") });
  }
  return out;
}

/** 域 2 第三枪：配置层动作词类型表；空＝还原内置；非空＝默认合并；{ clear: true }＝仅配置 */
export function setVerbTypes(hints, opts = {}) {
  const list = Array.isArray(hints) ? hints : [];
  if (list.length === 0) {
    verbTypesOverride = null;
    return;
  }
  const normalized = normalizeVerbHints(list);
  verbTypesOverride = opts?.clear ? normalized : [...VERB_TYPES, ...normalized];
}

/** 域 2 第三枪：配置层动作词总表（verbRe）；空＝还原内置；非空＝默认合并；非法正则跳过且不覆盖现状 */
export function setVerbRe(source, opts = {}) {
  if (typeof source !== "string" || source.length === 0) {
    verbReOverride = null;
    return;
  }
  ...
  verbReOverride = opts?.clear ? source : `(?:${VERB_RE_BUILTIN})|(?:${source})`;
}

function currentVerbHints() {
  return verbTypesOverride ?? VERB_TYPES;
}

// 每次 new RegExp(source, "gi")：禁止缓存实例（matchAll 要求 g；新实例 → 无 lastIndex 状态）
function currentVerbRe() {
  return new RegExp(verbReOverride ?? VERB_RE_BUILTIN, "gi");
}

function verbType(word, fullText) {
  for (const h of currentVerbHints()) {
    if (h.re.test(word)) {
      // 宽泛"执行/运行"在出现具体命令上下文时仍收窄为 command（本行留代码，不迁）
      if (h.type === "any" && /node\s+[\w./\\-]+\.(?:m?js|cjs)|npm\s+(?:run|exec|install)|run\s+|execute/i.test(fullText)) return "command";
      return h.type;
    }
  }
...
export function pairActionScopes(raw, clauseId, strict = false) {
  ...
  const verbs = [...text.matchAll(currentVerbRe())].map((m) => ({ index: m.index, word: m[0] }));
...
export function isApprovalText(text) {
  ...
  return approvalWordsRe().test(s) || actionWordsRe().test(s);
}
...
export function isSessionWideAskText(text) {
  return currentSessionWide().test(String(text || ""));
}
