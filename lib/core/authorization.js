...
// 疑问词表统一来自 intent.js（规则 22 意图判定与 12D 授权判定的唯一来源，2026-08-24 合并两套）
const DIRECTIVE_RE = /(?:请\s*)?(?:删除|移除|修改|编辑|替换|写入|写(?:下|好|完|成|作|文件)?|保存|另存为|创建|复制|移动|执行|运行|下载|提交|推送|备份|安装|卸载|清理|启动|停止|重建|修复|改(?:为|成|动|一下)?|补(?:上|齐|全|充|写)?|修(?:改|复|一下)?|做(?:好|完|一下)?|实施|推进|继续|处理|解决)/i;
const APPROVAL_RE = /(?:允许|同意|可以|是|好|确认|授权|执行|批准|去吧|开始|ok|yes)/i;
const REJECTION_RE = /(?:不允许|拒绝|不要|否|取消|不同意|no|No)/i;

// 2026-08-24（规则 22 粒度配套）：执行许可确认词。
// 只有「确认/同意/批准/授权/可以 + 即可/现在/马上/继续/开始/执行/落盘/去做/实施/进行……」
// 才构成执行许可；纯"确认方案/理解/方向"不构成。
export const APPROVAL_EXEC_RE =
  /(?:确认|同意|批准|授权|可以|允许|好|行|ok|yes)[^\n]{0,16}(?:即可|现在|马上|继续|开始|执行|落盘|去做|实施|进行|删除|修改|改|补|修|做|写入|创建|安装|发布|下载|提交|运行)/i;

// 纯方案/理解/方向确认（不构成执行授权）
const PLAN_ONLY_RE = /确认方案|确认理解|确认方向|确认无误|理解了|明白方向|了解|没问题|收到/i;

// 选项/自由文本里的明确动作词（用于 ask 结果判定：选择"执行/调整/修复…"即视为批准）
const EXEC_ACTION_RE = /执行|开始|做|改|补|修|实施|删除|修改|写入|创建|安装|发布|下载|提交|运行|升级|修复|补充|调整|推进|继续|落盘|保存|处理|解决/i;

const TYPE_HINTS = [
  { re: /删除|移除|remove|delete/i, type: "delete" },
  { re: /修改|编辑|替换|写入|写(?:入|文件|作|下|好|完|成)?|改(?:为|成)?|保存|另存为|edit|write|replace/i, type: "write" },
  { re: /备份|backup/i, type: "backup" },
  { re: /git\s+(push|commit)|提交|推送/i, type: "git" },
  // 命令类只在有明确命令上下文时推断；"执行/开始/做"等宽泛指令保持 any，避免把普通文件变更误限为 command
  { re: /命令|运行|执行\s*(?:命令|脚本|以下|程序)|run|execute|pwsh|bash|node\s+[\w./\\-]+\.(?:m?js|cjs)|npm\s+(?:run|exec|install)/i, type: "command" },
  ...
  { re: /审查|阅读|查看|核对|检查|审阅|复核|查阅|验证|对照|评估|分析|研究|排查|核实|梳理|调查|诊断/i, type: "analysis" }
];
 ...
 *  2. 截断可疑：裸含空格路径（无引号）只能提取到空格前的半截（"D:\example\workspace\..." → "d:/example"）
...
const VERB_TYPE_RE = [
  ...
  [/下载/i, "network"],
  [/执行|运行/i, "any"],
  [/写|保存|另存为|创建|复制|移动|修改|编辑|替换|改|补|修|做|安装|卸载|清理|修复|重建|启动|停止|调整|改进|优化|升级|迁移|整理|添加|增加/i, "write"]
];
const VERB_RE =
  /删除|移除|修改|编辑|替换|写入|写|保存|另存为|创建|复制|移动|备份|提交|推送|下载|执行|运行|安装|卸载|清理|修复|重建|启动|停止|调整|改进|优化|升级|迁移|整理|添加|增加|改|补|修|做|实施|推进|继续|处理|解决/g;
