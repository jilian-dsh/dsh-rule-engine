...
import { normalizeHandlerName, parseHandlerDecl, validateMeasure } from "./measure-kinds.js";

/**
 * hint_checks 的 10 个子键与顺序（＝迁移前 hintPatterns 内联正则的出现顺序）。
 * 置于模块顶部：hintPatterns 在模块求值期或运行期被调用时都不会踩 TDZ。
 */
export const HINT_CHECK_KEYS = [
  "ask",
  "time",
  "skill",
  "sensitive",
...
/**
 * 从检查文本提取机器可用的提示词。
 * 第三批（2026-09-21）：10 条判定词表迁配置层——经 patterns 的 **hint_checks 映射键**取词
 * （子键＝hint 名，见 patterns.js 的 PATTERN_MAP_KEYS.hint_checks）；本文件不再写死中文字面量。
 * L76/L77 两条（inline-command／bom-write）是工具名/参数形态，**不属规则词表**，仍留代码（用户裁定本枪不动）。
 * flags 口径：与迁移前逐条一致——旧内联正则**均带 `i`**（忽略大小写），故此处同样按 `"i"` 编译
 *（例：检查文本写 "Get-Date" 也必须命中 time）。
 */
function hintPatterns(checkText) {
  ...
  for (const key of HINT_CHECK_KEYS) {
    const re = reOf(key);
    if (re && re.test(checkText)) hints.push(key);
  }
  return [...new Set(hints)];
}

/**
 * 措施类型归一 + 声明解析（架构 v3 P1/P2，2026-09-10）：
 *   注册表（MEASURE_KINDS）+ 兼容别名表（LEGACY_HANDLER_ALIASES）+ 归一入口（kindOf）
 *   + 声明解析（parseHandlerDecl）+ 校验（validateMeasure）均在 ./measure-kinds.js
 *   —— 本文件不含任何规则号字面量。
...
export {
  ...
  kindOf,
  MEASURE_KINDS,
  LEGACY_HANDLER_ALIASES,
  parseHandlerDecl,
  validateMeasure
} from "./measure-kinds.js";
...
export function resolveHandler(rule, opts = {}) {
  ...
  const decl = parseHandlerDecl(body);
  // P2（架构正本 §三 L3）：声明非法（JSON 坏 / 空声明）**或 kind 未注册** → fail-closed（不执行）；
  // 错误详情由 understandRule 写入 config.measureError（此处只保证不产出可执行的 handler）。
  if (decl) return validateMeasure(decl) ? "" : normalizeHandlerName(decl.kind);
...
export function understandRule(rule, opts = {}) {
  ...
  // P2（2026-09-10）：个人层措施声明 → 措施实例 {kind, params}；kind 未注册 / 声明非法 → fail-closed
  const decl = parseHandlerDecl(rule.body || "");
  const measureError = validateMeasure(decl);
  const config = {
    ruleId: rule.index,
    title: rule.title,
    ...
    confidence,
    handler,
    measure: decl && !measureError ? decl : null,
