...
// 件 E（2026-09-22）：node 调用的 --dry-run 预览段（独立旗标：前有空白/行首，后为空白或串尾）。
// 只用于 isReadOnlyCommand 的段判定；写盘 cmdlet 由 MUTATING_CMD_RE 等三判据先行拦下。
const DRY_RUN_NODE_SEG_RE = /^node\s+(?=[^\r\n;|]*\s--dry-run(?:\s|$))/;
...
export function isReadOnlyCommand(command) {
  ...
  return segments.every((seg) => {
    ...
    // 件 E（2026-09-22）：本段是 node 调用且含**独立** --dry-run 旗标 → 只读（可证明的预览开关）。
    // 边界：仅认 node 开头的段，不是任意命令见旗标就放行；写盘三判据在上方已先行返回 false；
    // 不改 READONLY_CMD_RE 里已有的 --check／--test／--version，不把 scripts 目录一律当分析。
    if (DRY_RUN_NODE_SEG_RE.test(s)) return true;
...
export const PATTERN_MAP_KEYS = {
  ...
  self_cert_hints: {},
  // 检查文本 → 机器可用 hint 的判定词（understander.js 的 hintPatterns 消费；第三批 2026-09-21 迁入）。
  // 内置默认＝**语言无关最小集**（英文/通用标识符）；中文词表由 rule-engine.json 的
  // patterns.hint_checks 注入（本机同源见 test/helpers.mjs 的 TEST_PATTERNS_ZH.hint_checks）。
  hint_checks: {
    ask: "ask_user_question",
    time: "get-date",
    skill: "skill",
    sensitive: "git\\s+(push|commit)",
    backup: "backup",
    manual: "manual",
    retry: "retry",
    "runtime-verify": "runtime verif|mock",
    source: "URL|source",
    language: "language"
  },
  // 分点级意图判定词（intent.js 消费；第三批 2026-09-21 迁入）。
  // 值＝**语言无关最小集**（英文），中文词表经 rule-engine.json 的 patterns.intent_checks 注入
  // （本机同源见 test/helpers.mjs 的 TEST_PATTERNS_ZH.intent_checks）；flags 口径由消费方代码固定
  // （specific_action 与 at_action 用 "i"，其余空——见 intent.js 的 INTENT_CHECK_FLAGS）。
  intent_checks: {
    tag: "【(execute|plan|question|info)】",
    defer: "wait for (?:confirmation|approval)|hold on|not yet|do not (?:execute|run)|after confirmation|check with me first",
    conditional:
      "(?:after|once|when)\\s+\\w+(?:\\s+\\w+)?\\s+(?:is\\s+)?(?:done|complete[ds]?|finished)|(?:done|finished|complete[ds]?)\\s+(?:then|and then)|after\\s+\\w+ing",
    plan: "plan|proposal|suggest(?:ion)?|evaluat(?:e|ion)|analy[sz]e|design|propose|provide|give (?:me )?(?:a |an |the )?",
    specific_action:
      "write|save|delete|remove|modify|edit|change|create|install|publish|download|commit|push|run|fix|replace|rebuild|start|restart|stop|uninstall|improve|optimi[sz]e|enhance|upgrade|migrate|reorganize|adjust|implement|add|append|rewrite|refactor|debug|investigate|resolve|handle|update|deploy|verify|cross-check|review|read|inspect|check|audit|validate|compare|research|convert|transform|extract|restore|import|export|adopt|select|persist|clean\\s*up|merge|integrate|draft|build|produce|author|generate|survey|follow up",
    plan_executing:
      "(?:plan|steps?|checklist|arrangement)[^\\n]{0,16}?(?:execute|start|continue|proceed|carry out|implement)|(?:execute|start|continue)\\s+(?:the\\s+|this\\s+|that\\s+|above\\s+)?(?:plan|steps?)",
    plan_request:
      "give me|provide|design|evaluat(?:e|ion)|analy[sz]e|plan|propose|draft|write|explain|walk me through|how|where|need (?:a|an|one)",
    at_action: "read|revise|edit|write|convert|check|handle|extract|open|show|review|inspect|analy[sz]e|organize|proofread|cross-check|restore|update|delete|move|execute|process",
    status_exclude: "^(?:please\\s+)?(?:first\\s+|then\\s+)?(?:finish|restart|execute|complete|handle)\\s+\\S"
  },
  // 过度工程／重复打转表述检测词（overengineering.js 的 detectOverengineeringText 消费；第三批 2026-09-21）。
  // 内置默认＝**语言无关最小集**（英文）；中文词表经 rule-engine.json 的 patterns.overeng_checks 注入
  // （本机同源见 test/helpers.mjs 的 TEST_PATTERNS_ZH.overeng_checks）。
  // flags 口径留代码：extra_tech 用 "i"，其余三个为空（与迁移前逐条一致）。
  // AND／NOT 组合逻辑不进配置，仍在 detectOverengineeringText 代码里。
  overeng_checks: {
    extra_act: "while at it|by the way|extra|just in case|add it first|create one first|prevent future",
    extra_tech: "refactor|dependenc|abstract|compat|migrat|flag|validat|hash|full test|safety",
    recheck: "recheck|re-run the test|audit again|re-?verify",
    recheck_except: "new evidence|finding|fail|error|after (?:the )?change|after modification"
  },
  // 版本守卫判定词（version-guard.js 消费；第三批 2026-09-21）。
  // 内置默认＝**语言无关最小集**（英文）；中文词表经 rule-engine.json 的 patterns.version_guard_checks 注入
  // （本机同源见 test/helpers.mjs 的 TEST_PATTERNS_ZH.version_guard_checks）。
  // flags 口径留代码：path_mark 用 "i"，rule_bracket 空（与迁移前逐条一致）。
  // rule_bracket 必须保留一个捕获组（lineAnchor 仍取第 1 组）。
  version_guard_checks: {
    path_mark: "version|changelog|history|release notes",
    rule_bracket: "\\[rule\\s+([^\\]]+)\\]"
  },
  // 自证标记判定词（text-detect.js 的 isSelfCertified 消费；第三批 2026-09-21）。
  // 内置默认＝**语言无关最小集**（英文，无中文）；中文词表经 rule-engine.json 的 patterns.self_cert_checks 注入
  // （本机同源见 test/helpers.mjs 的 TEST_PATTERNS_ZH.self_cert_checks）。
  // 拼装公式仍在代码里：(?:${rule_word}\\s*${id}${suffix}\\s*${tail}|${tail}\\s*${rule_word}\\s*${id}${suffix})
  // id 的转义与 suffix（①–⑩／数字）同样留代码；本键 flags 空。
  self_cert_checks: {
    rule_word: "rule",
    tail: "done|completed|verified|checked|confirmed|compliant|addressed|already covered"
  }
};
...
/**
 * 配置层重置通知（2026-09-21 第三批）：setPatterns／resetPatterns 生效后，
 * 逐个调用注册的清理回调——供消费方清掉自己持有的"按旧配置编译"的正则缓存
 * （如 intent.js 的 intentReCache），避免缓存把配置注入挡在门外。
 * 注册的是幂等清理函数；重复注册不去重（各注册方只注册一次）。
 */
const patternResetCallbacks = [];
export function onPatternsReset(fn) {
  if (typeof fn === "function") patternResetCallbacks.push(fn);
}
function triggerPatternsReset() {
  for (const fn of patternResetCallbacks) {
    try {
      fn();
    } catch {
      // 清理回调失败不阻断配置生效
