...
export function isVerificationCommand(command) {
  if (typeof command !== "string") return false;
  if (/\b(?:node|npm|npx)\s+[^\n]*(?:run-all\.mjs|example-plugin-load\.mjs|audit-mount-consistency\.mjs|loader-smoke\.e2e\.mjs|verify-all\.mjs|health-audit\.mjs|check-tool-coverage\.mjs|publish-aptitude-check\.mjs|\.test\.mjs\b|--check|--test|--dry-run)\b/i.test(command)) return true;
