...
/** 判断命令是否为全量挂载审计脚本（规则 27 的“审计”侧） */
export function isAuditCommand(command) {
  return typeof command === "string" && /audit-mount-consistency\.mjs/i.test(command);
}

/** 审计输出是否明确通过（无 DUPLICATES FOUND 且出现通过标记） */
export function auditOutputPassed(output) {
  const text = String(output || "");
  return /MOUNT CONSISTENT|NO duplicate loader entry ids found/i.test(text) && !/DUPLICATES FOUND/i.test(text);
}

/** 审计输出是否明确发现重复 */
export function auditOutputFailed(output) {
  return /DUPLICATES FOUND/i.test(String(output || ""));
}
