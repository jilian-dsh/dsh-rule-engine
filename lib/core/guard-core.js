...
// ── 域 2 第五枪（2026-09-22）：规则 1 重试豁免词迁配置层（rule-engine.json 的 retryExempt 键）。
// 内置＝语言无关最小集；取词走 getter，禁止顶层缓存实例。──
const RETRY_EXEMPT_BUILTIN = /\b(?:retry|try\s+again|try\s+once\s+more|keep\s+trying)\b/.source;

let retryExemptOverride = null; // 合并后 source | null

/** 域 2 第五枪：空＝还原内置；非空＝默认合并；{ clear: true }＝仅配置；非法正则跳过且不覆盖现状 */
export function setRetryExempt(source, opts = {}) {
  if (typeof source !== "string" || source.length === 0) {
    retryExemptOverride = null;
    return;
  }
  try {
    new RegExp(source, "i");
  ...
  retryExemptOverride = opts?.clear ? source : `(?:${RETRY_EXEMPT_BUILTIN})|(?:${source})`;
}

function currentRetryExempt() {
  return new RegExp(retryExemptOverride ?? RETRY_EXEMPT_BUILTIN, "i");
}

/** 用户文本是否明确要求重试（规则 1 豁免判据；对齐 isSessionWideAskText 形态） */
export function isRetryExemptText(text) {
  return currentRetryExempt().test(String(text || ""));
}
...
function matchRule(cfg, ctx) {
  ...
  if (kindOf(cfg.handler) === "retry") {
    ...
    if (count >= 2 && isRetryExemptText(userText)) {
