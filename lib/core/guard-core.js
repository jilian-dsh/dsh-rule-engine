...
export const FREEDOM_SURFACE = [
  "只读工具：read / grep / glob / read_image / vision_* / 状态查询（job_list、engram_recall、esr_status 等）/ web_fetch / web_search",
  "分析临时区写：logs/.analysis-tmp/ 与 .backups/（脚本、下载、临时产物）",
  "验证类命令：单测 / 审计脚本 / 冷加载探针 / --dry-run / 只读 GET（gh api 无写方法）",
  "工作区内低风险新建：目标尚不存在的普通文件（非受保护名、非装配文件、非高风险入口）",
  "询问与展示：ask_user_question / todo_write / visualize / report",
  "已授权范围内的变更：本回合有执行分点且路径/类型落在授权 scope 内",
  "逃生门：/guard unlock（配置写保护，默认 10 分钟）与 /guard bypass（全部守卫暂停，有审计）"
];
/** 盲区：守卫看不见或判不准的（本引擎不是安全边界） */
export const BLIND_SPOTS = [
  "台面外直写：用户在终端 / 其他编辑器直接改文件，不经 DSH 工具链 → 守卫完全看不见",
  "语义判定误判：授权/意图判定权在词表与 LLM，词表未覆盖的表达可能被误判（可用 /guard label 打标）",
  "未分类工具：不在工具分类表里的工具按 unknown 处理（首次调用拦截，需用户放行）",
  "外部程序的文件变更：被 pwsh 调用的程序自己写文件，命令文本里看不到目标路径",
  "D 级自证类规则：引擎只提示，是否执行靠模型自觉（无机器强制）",
  "弱信号批评检测：偏宽形态只留痕不拦截（第三批后果分级后），真实批评可能漏判",
  "跨会话状态：授权/解锁是进程内状态，重启即失效；不跨会话继承"
];
