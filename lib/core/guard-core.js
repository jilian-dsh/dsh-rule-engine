 ...
 *  entryMarker：统一入口脚本名（默认 example-manual-write.mjs；localIntegrations.entryScript 可配）。 */
function isEntryChannelCommand(cmd, entryMarker = "example-manual-write.mjs") {
...
export function guardDecision(state, exec, now = Date.now(), opts = {}) {
  ...
  if (!unlock && (name === "pwsh" || name === "bash")) {
    const cmd = commandText(args) || "";
    const entryMarker = state.localIntegrations?.entryScript || "example-manual-write.mjs";
...
function matchRule(cfg, ctx) {
  ...
  if (cfg.handler === "rule13a-backup") {
    ...
    if ((name === "pwsh" || name === "bash") && isEntryChannelCommand(cmd, state.localIntegrations?.entryScript || "example-manual-write.mjs")) return null;
  ...
  if (cfg.handler === "rule12b-skill") {
    if (name === "skill") {
      ...
      if (SKILL_EXEMPT.has(skillName) || ((state.localIntegrations?.manualExempt?.skills || []).includes(skillName))) return null;
  ...
  if (cfg.handler === "rule12a-approval") {
    if (isSensitiveToolCall(name, args, sessionIdOf(exec))) {
      ...
      if (p && (MANUAL_PATH_RE.test(p) || ((state.localIntegrations?.manualExempt?.paths || []).some((pp) => p.toLowerCase().includes(pp.toLowerCase()))))) return null;
