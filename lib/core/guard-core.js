...
const COVERED_MUTATION_TOOLS = new Set(["edit", "write", "str_replace_editor", "pwsh", "bash"]);
...
function looksLikeFileMutation(name, args) {
  const a = args || {};
  return Boolean(a.file_path || a.path || a.command || a.code);
}
...
function matchRule(cfg, ctx) {
  ...
  // 规则 27：装配变更后必须先通过全量审计，才能继续装配（C 时序）
  if (cfg.handler === "rule27-mount-audit") {
    if (isAssemblyMutationTool(name, args)) {
      if (session.mountDirty && !session.mountAuditPassed) {
        return makeHit(cfg, "【硬拦截】检测到插件装配已变更但未通过全量审计（规则 27）：请先运行 node _extract/audit-mount-consistency.mjs --profile <p> 并通过后再继续装配");
      }
    }
    return null;
  }
