...
// ── version_guard_checks 取词（第三批，2026-09-21）──
// 两条判定词迁 patterns.version_guard_checks；flags 口径留代码（path_mark 用 "i"，rule_bracket 空）。
// 缓存不被模块级锁死：onPatternsReset 在 setPatterns／resetPatterns 生效后清空本缓存。
const VG_CHECK_FLAGS = { path_mark: "i" };
let vgReCache = new Map();
function vgRe(key) {
  if (vgReCache.has(key)) return vgReCache.get(key);
  const src = patMap("version_guard_checks")[key];
  let re = null;
  if (typeof src === "string" && src.length > 0) {
    try {
      re = new RegExp(src, VG_CHECK_FLAGS[key] || "");
    } catch {
      re = null;
    }
  }
  vgReCache.set(key, re);
  return re;
}
onPatternsReset(() => vgReCache.clear());
...
export function isVersionedFile(p) {
  ...
  return vgRe("path_mark")?.test(p) === true;
}
...
function lineAnchor(s) {
  const rule = vgRe("rule_bracket")?.exec(s);
