...
const SELF_CERT_REASONS = {
  "14": "请按规则 14 一次性完整汇报，区分事实与推断",
  "22": "检测到「我记下了」类空话；正确动作是落盘执行并汇报",
  "23": "交付/完成声明未附运行时验证证据",
  "16": "请按规则 16 用绑定检查格式提建议（或给档位建议并说明理由），避免频繁打断",
  "12E": "请按规则 12E 先确认界面真实存在并声明能力边界",
  "26": "请按规则 26 确认正式 Release Asset（release.assets）并附 tgz/zip",
  "10": "请按规则 10 自证：如实说明默认不自动继承，按优先级恢复并标注来源",
  "12C": "请按规则 12C 自证：需代理先告知、失败即停、下载后校验、排查先测连通性",
  "13B": "请按规则 13B 自证：会话替换前用户完全退出、三层验证一次跑完、mtime 判断",
  "15": "请按规则 15 自证：不凭 mtime/文件名判断新旧，无法确定版本/用途时保留并询问",
  "19": "请按规则 19 自证：按①-⑧沉淀并当次汇报正文同步位置",
  "28": "请按规则 28 自证：新文件按工作区目录索引归类，不确定则查 README 或放 _inbox",
  "29": "请按规则 29 自证：重启前验证依赖闭包（关键包非空/顶层 junction 完整/无 dangling）",
  "30": "请按规则 30 自证：执行前证明不破坏依赖+已授权，完成后附验证证据",
  "31": "规则 31 自证：请写明「验证目标→撞了什么/为什么撞→结论来源（手册位置/源码行号）」；不连续盲试"
};

/** 取某条款的自证提示（正则经 patMap 从配置层取；无配置/无文案 = null） */
function selfCertHint(ruleId) {
  const src = patMap("self_cert_hints")[String(ruleId)];
  const reason = SELF_CERT_REASONS[String(ruleId)];
  if (typeof src !== "string" || !src || !reason) return null;
  return { re: new RegExp(src, "i"), reason };
}
  ...
  if (cfg22 && session.lastUserText) {
    ...
    if (sig.suspect === "strong" && !isQuoteOrParaphraseContext(critText, criticismShapeRe())) {
      hits.push({
        ...
        mode: "criticism",
        suspect: "strong",
        reason: "用户消息命中批评/指错疑似信号（强形态；规则 22②/手册 5b-2，待裁决）：批评不构成任何授权——本回合写类工具已冻结，请按四段模板回应：① 停止；② 归因四问；③ 三件套（事实/原因/方案）；④ 等指令"
      });
      ...
      hits.push({
        ...
        mode: "criticism",
        suspect: "weak",
        reason: "用户消息疑似批评/质问形态（规则 22②/手册 5b-2，待裁决）：若确属批评——不构成任何授权，本回合写类工具已冻结，请按四段模板回应（停止/归因四问/三件套/等指令）"
      });
    }
  }
