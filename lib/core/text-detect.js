...
// 用户批评形态（规则 22② 机器提示，2026-09-02）：骂词/质问句型/连续问叹符——强信号防误报；
...
export function detectViolations({ configs, session, text, reasoningText = "", mountRevision = 0 }) {
  ...
  // 规则 22② 机器提示（2026-09-02）：用户消息含批评/质问形态 → 审计+注入"批评≠授权"提醒
  const cfg22 = byId.get("22");
  if (cfg22 && session.lastUserText && CRITICISM_RE.test(session.lastUserText) && !isQuoteOrParaphraseContext(session.lastUserText, CRITICISM_RE)) {
    hits.push({
      ruleId: "22",
      title: cfg22.title,
      kind: "correct",
      reason: "用户消息含批评/质问形态（规则 22②/手册 5b-2）：用户批评不构成任何授权——被批评后应停、按归因四问产出三件套、等指令；禁止因批评启动任何变更"
    });
  }
