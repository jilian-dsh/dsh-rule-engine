...
export function detectViolations({ configs, session, text, reasoningText = "" }) {
  ...
  if (mountCfg && session.mountDirty && !session.mountAuditPassed && /重启|装配|挂载/.test(text) && !/审计通过|全量审计|MOUNT CONSISTENT|DUPLICATES FOUND|audit-mount-consistency/.test(text)) {
    hits.push({
      ruleId: "27",
      title: mountCfg.title,
      kind: "self-certify",
      reason: "规则 27：插件装配已变更且未通过全量审计，重启/继续前请先运行 audit-mount-consistency.mjs"
    });
  }
