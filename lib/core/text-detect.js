...
export const CRITICISM_SHAPE_RE = /(?:[？?]{3,}|[！!]{4,})/;
...
export function detectViolations({ configs, session, text, reasoningText = "", mountRevision = 0, rule5Window = 3 }) {
  ...
  if (cfg22 && session.lastUserText) {
    ...
    if (sig.suspect === "strong" && !isQuoteOrParaphraseContext(critText, CRITICISM_SHAPE_RE)) {
      hits.push({
        ruleId: "22",
        title: cfg22.title,
        kind: "self-certify",
    ...
    } else if (sig.suspect === "weak" && !isQuoteOrParaphraseContext(critText, CRITICISM_WEAK_RE)) {
      hits.push({
        ruleId: "22",
        title: cfg22.title,
        kind: "self-certify",
