...
/** v0.5.7 后续（用户批准）：unknown 工具白名单持久化文件（"允许使用 X" → 重启不丢） */
export function toolsWhitelistFilePath() {
  return join(dshHome(), "rule-engine-tools.json");
}

/** 0.5.15：回合末裁决卡片持久化文件（卡片 + 判例登记 → 重启不丢，历史按钮/判例恢复） */
export function turnCardsFilePath() {
  return join(dshHome(), "rule-engine-turn-cards.json");
}
