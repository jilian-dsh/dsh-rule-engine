...
// → 启动即审计告警（失败不阻断加载，但留下可观测证据——无声守卫消失风险的对策）
{
  ...
  if (typeof script === "string" && script) {
    // 入口脚本通常与本插件 scripts/ 同目录或工作区；按 DSH_HOME 与工作区两处解析存在性——
    // 找不到文件时告警（可能是用户配置了尚未部署的入口名）
    const candidates = [];
    try {
      candidates.push(`${dshHome || ""}${dshHome ? "\\" : ""}scripts\\${script}`);
      candidates.push(`${process.cwd?.() ?? ""}${process.cwd?.() ? "\\" : ""}scripts\\${script}`);
    } catch { /* 路径构造失败不阻断 */ }
    const found = candidates.some((c) => { try { return existsSync(c); } catch { return false; } });
    if (!found) {
      audit({
        ...
        name: "启动自检：统一入口脚本不存在",
        event: "startup",
        reason: `localIntegrations.entryScript 指向的脚本（${script}）在预期路径未找到——本机守卫仍按配置工作，但入口通道可能失效；请检查配置或部署该脚本`,
        session: "global"
      });
    }
  }
