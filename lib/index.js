...
export function handleSessionEvent(ctx, session, event) {
  ...
  if (event.type === "tool/result") {
    ...
    if (!isError) {
      const output = extractToolOutput(d);
      if (/\bALL TESTS PASSED\b|RESULT:\s*PASS/.test(output)) {
  ...
  if (event.type === "assistant/message") {
    ...
    if (/完成|已通过|已修复|搞定|验证通过|全部通过|修复完成|落盘完成/.test(text)) {
      ...
      if (!hasPass) {
        ...
        maybeInject(ctx, sid, { ruleId: "23", reason: "规则 23④：完成/通过类声明需同会话近期验证记录（测试全绿或冷加载 PASS），缺失已记审计——请先运行验证并附证据再宣布完成" });
      }
    }
    ...
    // 注入噪音治理 v0.5.6（建议1/B1 聚合注入）：同回合多条违规 → 聚合为一条注入（明细全在 /guard log）；
    // 单条仍按逐规则投递（保留完整提示文本与逐规则计数上限）；聚合注入用伪规则 __self-cert 计数。
    if (violations.length > 1) {
      const briefs = violations
        .map((v) => `规则 ${v.ruleId}（${String(v.reason || "").slice(0, 60)}）`)
        .join("；");
      maybeInject(ctx, sid, {
        ruleId: "__self-cert",
        reason: `本轮检出 ${violations.length} 项待自证：${briefs}——请按 /guard log 明细一次性自证`
      });
    } else if (violations.length === 1) {
      maybeInject(ctx, sid, violations[0]);
    }
