...
import { setFormats } from "./core/formats.js";
import {
  askQuestionCoreText,
  askQuestionText,
  askResultApproved,
...
import { APPROVE_TYPES, normalizePath, setTypeHints, setVerbTypes, setVerbRe, setDelegationMarker, setSessionWide } from "./core/authorization.js";
import { setLexicons, approvalExecRe } from "./core/lexicon.js";
// C4（批 3，2026-09-15）：质量账本查询入口（/guard quality）。机制自 0.6.3 已在 core/quality-ledger.js，
// 但 lib/ 内此前零引用（grep "quality" = 0 命中）——机制写了却没人能用，本批补入口。
import { loadLedger, qualityTrend } from "./core/quality-ledger.js";
import { getMessage } from "./messages.js";
import { getBlindSpots, getFreedomSurface, setGuardFreedom, setRetryExempt } from "./core/guard-core.js";
import { detectLang, getLang, getLangSource, pickLang } from "./core/lang.js";
import { DELIVERY_RE, detectViolations, extractAssistantText, hasExplicitExecWord, setCriticismPersonal, setInjectCommand, isInjectCommandText, setWhitelistAllow, currentWhitelistAllow } from "./core/text-detect.js";
...
// 未归类工具策略的上一次读成功值：初值＝启动时读到的那一份；调用时现读失败（ok 为 false）即沿用此值
let unknownPolicyLastGood = pluginConfig.unknownPolicy;
// §1.3 行为闸判定（纯函数，2026-09-08 第二批 A″）：批评疑似回合冻结写类工具——返回拒绝原因或 null
// C2-C 档（批 3，2026-09-15）：新增 opts.bypassActive —— `/guard bypass` 的语义是「全部守卫暂停」
//（guard status 文案原文），而本闸此前只查 turn.criticismFrozen、不检查 state.bypassUntil →
// 用户显式放行后仍被 A″ 冻结，语义不一致。bypassActive 由调用方从 state.bypassUntil 计算后传入，
// 本函数保持**纯函数**（可单测）；窗口内的留痕由 guard 路径既有的 bypass-action 审计统一负责（M2）。
export function criticismFreezeDecision(session, toolName, args, opts = {}) {
  if (opts.bypassActive) return null;
...
/** C3（批 3，2026-09-15，用户裁定方案 A）：**是否冻结**本回合写类工具（纯函数，可单测）。
 *  由来：置位逻辑原对强弱信号**不对称**——弱信号已做反向检查（hasExplicitExecWord，只认动作词），
 *  强信号却无条件冻结 → 「批评 + 明确指令」的混合消息会把用户同时下达的指令一起冻掉，
 *  而那条指令本身是**完整授权**（许可词 + 执行语，规则 22 的授权定义）。
 *  判据：强信号下只有 `approvalExecRe()`（许可词 + [0,16] 字符 + 执行语，缺一不可）才豁免——
 *  比弱信号的反向检查更严，避免「批评里提个动作词就算授权」的过宽。
 *  与 criticismFreezeDecision 分工：本函数管「要不要冻」，那个管「冻了之后拦不拦」。
 *  @returns {boolean} true = 冻结本回合写类工具
 */
export function criticismFreezeBySignal(suspect, userText) {
  if (suspect !== "strong") return false;
  return !approvalExecRe().test(String(userText || ""));
}
// C4：启动时合并配置层 typeHints（本机扩展授权类型；默认空=仅内置 8+archive 类）
setTypeHints(pluginConfig.typeHints);
// 域 2 第三枪（2026-09-22）：动作词表配置层注入（rule-engine.json 的 verbTypes／verbRe 键）——
// 空＝还原内置；非空＝默认与内置合并；{ clear: true }＝仅配置（同 setTypeHints 的 Override 语义）。
setVerbTypes(pluginConfig.verbTypes);
setVerbRe(pluginConfig.verbRe);
// 域 2 第四枪（2026-09-22）：范围标记正则配置层注入（rule-engine.json 的 delegationMarker／sessionWide 键）——
// 空＝还原内置；非空＝默认与内置合并；{ clear: true }＝仅配置。
setDelegationMarker(pluginConfig.delegationMarker);
setSessionWide(pluginConfig.sessionWide);
// 域 2 第五枪（2026-09-22）：规则 1 重试豁免词配置层注入（rule-engine.json 的 retryExempt 键）。
setRetryExempt(pluginConfig.retryExempt);
// 域 3 第一枪（2026-09-22）：naturalMode 五条正则配置层注入（rule-engine.json 的 naturalMode 键）。
setNaturalMode(pluginConfig.naturalMode);
// 域 3 第二枪（2026-09-22）：注入文案命令词配置层注入（rule-engine.json 的 injectCommand 键）。
setInjectCommand(pluginConfig.injectCommand);
// 域 3 第三枪（2026-09-22）：白名单口令正则配置层注入（rule-engine.json 的 whitelistAllow 键）。
setWhitelistAllow(pluginConfig.whitelistAllow);
...
// 非法键/非法正则被拒绝并记审计（不静默半套生效）。
{
  ...
  // (c) 格式描述对象（2026-09-21）：规则容器格式层注入（rule-engine.json 的 **formats** 键，独立于 patterns）。
  // 五键＝rule_header／section／free_zone（起止算一处）／level／elements；语义同 setPatterns（缺省不干预、按键替换）。
  const fmtRes = setFormats(pluginConfig.formats);
  if (fmtRes.rejected.length > 0) {
    audit({
      kind: "format-config-rejected",
      rule: "__format-config",
      name: "格式描述注入：部分键被拒绝",
      event: "startup",
      reason: `formats 配置被拒绝的键：${fmtRes.rejected.map((r) => `${r.key}(${r.reason})`).join(", ")}；生效键：${fmtRes.applied.join(", ") || "无"}`,
      session: "global"
    });
  }
}
...
function maybeInject(ctx, sessionId, violation) {
  ...
  // 域 3 第二枪（2026-09-22）：判据迁配置层（injectCommand 键，取词见 text-detect.js isInjectCommandText）
  if (isInjectCommandText(violation?.reason)) {
    audit({
...
export function handleSessionEvent(ctx, session, event) {
  ...
  if (event.type === "turn/end") {
    ...
    if (m8Cfg.enabled === true && m8Marker && s.turn.manualWriteSeen && !s.turn.engramStoreSeen) {
      ...
      // D（2026-09-13）：事中已提示过（落盘那一刻）→ 此处只留审计，不重复投递；
      // 未开启 hintOnWrite 或事中未提示时，此处仍是兜底投递（默认关时行为与改动前一致）。
      if (!s.turn.m8Hinted) {
        maybeInject(ctx, sid, {
          ruleId: "__engram-gap",
          reason: getMessage("memory.sediment-missing")
    ...
    // A3（规则 2 回合末按 lane 复核）：pendingRule2 = [{ lane, reason }]——
    // 本回合最终调用过 Get-Date 时只撤 lane=getdate 那条（有该类才留痕 rule2-resolved，无则不写）；
    // 其余各类各记一条 correct（带 lane），原因合成一条提醒、只投递一次（同会话同规则只投递一次）。
    const p2 = s.turn.pendingRule2;
    if (Array.isArray(p2) && p2.length > 0) {
      const withdrawn = s.turn.getDateSeen ? p2.filter((p) => p.lane === "getdate") : [];
      const deliverable = p2.filter((p) => !withdrawn.includes(p));
      if (withdrawn.length > 0) {
        audit({
          ...
          name: "规则2回合末复核（已核对）",
          event: "turn/end",
          reason: "回合内最终已调用 Get-Date（getDateSeen=true）——lane=getdate 待投递撤销，不投递",
          session: sid
        });
      }
      ...
      if (deliverable.length > 0) {
        maybeInject(ctx, sid, { ruleId: "2", reason: deliverable.map((p) => p.reason).join("；") });
      }
  ...
  if (event.type === "user/message") {
    ...
    for (const hit of text.matchAll(currentWhitelistAllow())) {
      const toolName = hit.slice(1).find(Boolean); // 域 3 第三枪：默认合并后有两组捕获（内置＋配置），禁止 hit[1]
  ...
  if (event.type === "tool/call") {
    ...
    if (toolName === "ask_user_question") {
      s.turn.askSeen = true;
      // D2 修复（2026-09-10）：写**会话级**（不随 turn/start 清空）；at 供陈旧值保护
      s.pendingAsk = { callId: d.callId, questions: args?.questions || [], at: Date.now() };
    }
  ...
  if (event.type === "tool/result") {
    ...
    if (!isError && pendingCall) {
      ...
      // C1（批 3，2026-09-15）：判据由「命令文本含 entryMarker」收紧为
      // 「命令含 entryMarker **且 结果文本含落盘成功标志**」。
      // 原判据不区分「跑了」与「跑成」：--dry-run（不写入）、写隔离夹具目录、脚本中途失败
      // （工具层不标 isError）都会置位 manualWriteSeen → turn/end 误判 __engram-gap 并要求
      // 同回合 engram_store（2026-09-15 会话实证：dry-run 亦被提示）。
      // 成功标志走配置 successMarker（与 entryMarker 同风格；未配置时用统一入口的既有标志）。
      const successMarker = m8Cfg2.successMarker || "MANUAL_WRITE_OK";
      const m8ResultText = String(
        resultBlock?.content?.map?.((c) => c?.text || "").join(" ") ||
        d?.message?.content?.map?.((c) => c?.text || "").join(" ") ||
        ""
      );
      if (m8Cfg2.enabled === true && marker && (pendingCall.name === "pwsh" || pendingCall.name === "bash")
        && cmd.includes(marker) && m8ResultText.includes(successMarker)) {
        s.turn.manualWriteSeen = true;
        // D（2026-09-13）：提示前移——落盘被识别的这一刻就提示（陈述式），不再等到 turn/end
        //（turn/end 之后模型已停止行动，提示只能影响下一回合＝事后才说，正是 M8 反复断链的结构原因）。
        // 分工：此处＝事中提示（预防）；turn/end 的 engram-gap 审计保留，兜底投递仅在此处未提示时进行。
        // 防重复：maybeInject 的闸是「同规则最多 3 次」（phase1f-inject.test.mjs 用例 5 为证），
        // 并非一次即止 —— 故必须由回合标记 m8Hinted 保证「每个回合最多提示一次」。
        // 默认关（hintOnWrite），与 M8 系列一贯的「显式启用」风格一致。
        if (m8Cfg2.hintOnWrite === true && !s.turn.engramStoreSeen && !s.turn.m8Hinted) {
          s.turn.m8Hinted = true;
          maybeInject(ctx, sid, {
            ruleId: "__engram-gap",
            reason: getMessage("memory.sediment-hint")
          });
        }
      }
    ...
    // D2 修复（2026-09-10）：会话级待决 ask + 陈旧值保护（30 分钟未答视为作废，防跨回合误登记）
    if (pending && (!pending.at || Date.now() - pending.at <= 30 * 60 * 1000)) {
      ...
      if (askResultApproved(result)) {
        ...
        // A2 修复（2026-09-10）：原用单数版 inferPathPrefixFromText——只记一个路径且取"最长"。
        // 探针实测：多路径问题文本下"最长"往往是次要路径（如"去向 .backups/trash-x"）而非主路径，
        // 主操作路径丢失 → 授权匹配必然失败。改为复数版：pathPrefix 取首个（按文本出现顺序=主路径，
        // 与用户消息侧 scopesFromIntents 的复数口径对齐），全部路径记入 pathPrefixes 供 authMatches 任一匹配。
        const pathPrefixes = inferPathPrefixesFromText(scopeText || qText);
        const pathPrefix = pathPrefixes[0] || "";
        // 0.5.10 建议1①：ask 答复结构化——操作类型从答复文本推断（write/command/any），不再一律 any
        // （修复"弹窗答复接不住真想操作"的粒度问题——V476AL 同族）
        // 执行单 3-4 已选 (b)（2026-09-22）：类别优先取「被拦那次调用」的操作类型（lastDeniedType，
        // 30 分钟内有效）；文本若还推出其它具体类（去 any／analysis 后与首类不同），按 ASK_EXTRA_ORDER
        // **只补第一条**即停（每类一条，且不让 classifyAskScopeType 的优先序把 command 丢掉）。
        // 没有 lastDeniedType 时才用文本兜底：文本同时命中 command 与 git 就这两类各记一条，
        // 否则仍按 classifyAskScopeType 单条记。
        const ASK_EXTRA_ORDER = ["delete", "backup", "git", "archive", "skill", "write", "command", "network"];
        const deniedType = s.lastDeniedType && Date.now() - (s.lastDeniedAt || 0) <= 30 * 60 * 1000
          ? s.lastDeniedType
          : null;
        const textTypes = inferTypesFromText(scopeText || qText).filter((t) => t !== "any" && t !== "analysis");
        const askTypes = [];
        if (deniedType) {
          askTypes.push(deniedType);
          for (const t of ASK_EXTRA_ORDER) {
            if (t !== deniedType && textTypes.includes(t) && !askTypes.includes(t)) {
              askTypes.push(t);
              break; // 只补第一条不同的类即停（其余文本类不登记，避免凭空放宽）
            }
          }
        } else if (textTypes.includes("command") && textTypes.includes("git")) {
          askTypes.push("command", "git");
        } else {
          askTypes.push(classifyAskScopeType(scopeText || qText));
        }
        const sessionWide = isSessionWideAskText(scopeText) || isSessionWideAskText(selectedText);
        // authRecord 保持与改前同作用域：下方 s.turn.scopes.push(authRecord) 仍指向**第一条**授权记录
        // （多类时首条即「被拦那次调用」的类；单类时与改前完全一致）。
        let authRecord = null;
        let firstRecord = null;
        for (const t of askTypes) {
          const slice = { type: t, pathPrefix, source: "ask" };
          if (pathPrefixes.length > 1) slice.pathPrefixes = pathPrefixes;
          if (sessionWide) slice.expiresAt = Date.now() + 12 * 60 * 60 * 1000;
          else if (!pathPrefix) slice.expiresAt = Date.now() + 2 * 60 * 1000;
          recordAuthorization(state, sid, slice);
          if (!firstRecord) firstRecord = slice;
        }
        authRecord = firstRecord || { type: classifyAskScopeType(scopeText || qText), pathPrefix, source: "ask" };
        if (deniedType) {
          s.lastDeniedType = null;
          s.lastDeniedAt = 0;
        }
        ...
        for (const hit of whtText.matchAll(currentWhitelistAllow())) {
          const wname = hit.slice(1).find(Boolean); // 同上：两组捕获，禁止 hit[1]
          if (!wname) continue;
      ...
      s.pendingAsk = null; // D2 修复（2026-09-10）：清理**会话级**待决 ask
    }
    return;
  }
  ...
  if (event.type === "assistant/message") {
    ...
    if (critHit) {
      ...
      // C3（批 3，2026-09-15，用户裁定方案 A）：冻结判定抽为纯函数——强信号下只有**完整执行许可**
      // （许可词 + 执行语）才豁免，避免把用户同一条消息里同时下达的指令一起冻掉。
      const freezeCrit = criticismFreezeBySignal(critHit.suspect, s.turn.userText || "");
      if (freezeCrit) {
        s.turn.criticismFrozen = true;
      } else if (critHit.suspect === "weak") {
        ...
        // C3：强信号 + 完整执行许可 → 不冻结，但**不静默放行**（留痕 + 陈述式提示；用户裁定「给一句提示」）
        maybeInject(ctx, sid, { ruleId: "22", reason: getMessage("criticism.strong-permitted") });
      }
      audit({
        ...
        name: "批评疑似信号（A″）",
        event: "assistant/message",
        reason: critHit.suspect === "strong"
          ? (freezeCrit ? "信号=strong——本回合写类工具冻结（A″ 强信号闸）" : "信号=strong ＋ 检出完整执行许可——不冻结（C3 方案 A 豁免，已提示）")
          : "信号=weak——不冻结（后果分级：留痕+提示）",
        session: sid
      });
    }
    // A3（规则 2 按 lane 记账）：pendingRule2 = [{ lane, reason }]，同一回合同一 lane 只记一条
    // （同会话同规则只投递一次，同类重复记账只会被去重闸丢掉）；记账后从 violations 摘除，投递归回合末。
    const rule2s = violations.filter((v) => v.ruleId === "2");
    for (const v of rule2s) {
      const pending = (s.turn.pendingRule2 ||= []);
      const lane = v.lane || "evidence";
      if (!pending.some((p) => p.lane === lane)) pending.push({ lane, reason: v.reason });
...
export const COMMAND_SPECS = [
  ...
  // C4（批 3，2026-09-15）：质量账本查询——机制在 core/quality-ledger.js，本项补的是**缺失的入口**
  { name: "quality", args: "", desc: "质量账本趋势（同类任务签名 × 返工/介入/摩擦/token）" },
...
export function parseCommand(rawInput) {
  ...
  // C4（批 3，2026-09-15）：质量账本查询（白名单登记——未登记即落 invalid，见本函数上方注释）
  if (/^quality$/i.test(text)) return { kind: "quality" };
...
async function executeGuard(ctx, invocation) {
  ...
  switch (command.kind) {
    ...
    case "quality": {
      // C4（批 3，2026-09-15）：质量账本查询入口。
      // 语义：账本默认关（rule-engine.json 的 qualityLedger.enabled=false）→ 无数据时**如实说明**、不伪造趋势；
      // 有数据时按签名分组各出趋势摘要。纯旁路统计：不拦截、不评分、不上传（数据留本机 ~/.dsh/quality-ledger.jsonl）。
      const qCfg = loadPluginConfig().qualityLedger || {};
      const qRows = loadLedger({ config: qCfg });
      if (qRows.length === 0) {
        return {
          kind: "success",
          text: qCfg.enabled
            ? "质量账本已开启，但暂无记录（账本只在任务收尾时记录；纯旁路统计，不拦截）"
            : "质量账本未开启（rule-engine.json 的 qualityLedger.enabled=false）——开启后按同类任务签名统计返工/介入/摩擦/token 趋势"
        };
      }
      const bySig = new Map();
      for (const r of qRows) bySig.set(r.sig, (bySig.get(r.sig) || 0) + 1);
      const top = [...bySig.entries()].sort((a, b) => b[1] - a[1]).slice(0, 8);
      const body = top.map(([sig, n]) => `  ${sig}  ${n} 单  ${qualityTrend(sig, { config: qCfg }).summary}`).join("\n");
      return { kind: "success", text: `质量账本：${qRows.length} 单 / ${bySig.size} 类签名\n${body}` };
    }
    case "approve": {
...
export function apply(ctx) {
    ...
    function* () {
      yield ctx.tools.guard((exec) => {
        ...
        if (hit) {
          ...
          // 执行单 3-4 已选 (b)（2026-09-22）：记下被拦那次调用的操作类型（ask 通过时按它登记类别；any 不写）
          {
            const op = operationOf(exec?.name, exec?.arguments || {});
            if (op && op.type && op.type !== "any") {
              const sDenied = getSessionState(state, sessionIdOfExec(exec));
              sDenied.lastDeniedType = op.type;
              sDenied.lastDeniedAt = Date.now();
            }
          }
  ...
  // 未归类工具首调策略（默认口径）：调用时现读配置——
  //   缺键与 "off" = 放行并留审计；"ask" = 官方 pre-execute 询问（approval 弹窗，allowed-once 才续行，
  //   无审批通道时由官方 fail-closed）；"deny" = 物理拒绝 + 提示；仅字符串取值参与匹配，
  //   先去首尾空格再转小写，其它类型与认不出的取值一律按拒绝处理。
  //   "ask" 不作默认取值，须先实弹验证审批弹窗真实出现（K-02 门禁）后方可启用。
  //   读失败（ok 为 false）沿用上一次读成功的值，初值＝启动时读到的那份。
  ctx.on("tools/pre-execute", async (exec, next) => {
    try {
      ...
      // 调用时现读：读成功即刷新上一次成功值；读失败（ok 为 false）或抛错则沿用上一次读成功的值
      let policyValue = unknownPolicyLastGood;
      try {
        const fresh = loadPluginConfig();
        if (fresh && fresh.ok === false) {
          // 读失败：沿用上一次读成功的值（初值＝启动时读到的那份）
        } else {
          policyValue = fresh?.unknownPolicy;
          unknownPolicyLastGood = policyValue;
        }
      } catch {
        // 抛错同样沿用上一次读成功的值
      }
      // 归一：缺键与 "off" 放行，"ask" 询问，"deny" 与认不出的取值拒绝
      const policyRaw = policyValue;
      // 仅字符串取值参与匹配：先去首尾空格再转小写；其它类型一律按认不出处理
      const policyNorm = policyRaw === undefined ? "off" : (typeof policyRaw === "string" ? policyRaw.trim().toLowerCase() : null);
      const policyUnrecognized = !(
        policyRaw === undefined
        || (typeof policyRaw === "string" && (policyNorm === "off" || policyNorm === "ask" || policyNorm === "deny"))
      );
      const policy = policyNorm === "off" ? "allow" : (policyNorm === "ask" ? "ask" : "deny");
      const unknownArgs = summarizeArgs(exec?.arguments);
      const approvePath = `（批准路径：官方弹窗批准 / 用户明确"允许使用 ${exec.name}"写入会话白名单；长期规则请补充工具分类表）`;
      if (policy === "allow") {
        // 放行支：只留一条审计，不写回合卡片、不改 lastActive、不记 deniedKeys
        audit({
          kind: "unknown-tool-allow",
          rule: "__unknown-tool",
          name: "未归类工具首调处置",
          event: "tools/pre-execute",
          tool: exec?.name,
          args: unknownArgs,
          reason: `工具 ${exec.name} 不在规则引擎分类表：按 unknownPolicy=${policyRaw === undefined ? "缺键" : policyNorm} 放行${approvePath}`,
          session: sessionIdOfExec(exec)
        });
        return next();
      }
      audit({
        ...
        tool: exec?.name,
        args: unknownArgs,
        reason: policyUnrecognized
          ? `工具 ${exec.name} 不在规则引擎分类表：取值 ${JSON.stringify(policyRaw)} 无法识别，按 deny 处理${approvePath}`
          : `工具 ${exec.name} 不在规则引擎分类表：按 unknownPolicy=${policyNorm} 首调处置${approvePath}`,
        session: sessionIdOfExec(exec)
      });
      if (policy === "ask") {
        return {
      ...
      // 件 B（2026-09-22）：未归类工具 deny 同样补卡片数据源（ask 分支不写卡片）
      try {
        const sCard2 = getSessionState(state, sessionIdOfExec(exec));
        (sCard2.turn.cardHits ||= []).push({
          tool: exec?.name || "",
          args: summarizeArgs(exec?.arguments || {}),
          ruleId: "__unknown-tool",
          title: "未归类工具首调处置",
          reason: `工具 ${exec.name} 未归类（unknownPolicy=${policy}）`,
          errId: ""
        });
        state.lastActive = [{ ruleId: "__unknown-tool", title: "未归类工具首调处置", reason: `工具 ${exec.name} 未归类` }];
      } catch { /* 同上：卡片失败不影响拦截 */ }
      // 件 C（2026-09-22）：未归类 deny 也记入 deniedKeys（同 L1915–1920）——不记则失败回执仍给
      // retryCounts 加一，第 4 次会从本闸文案变成规则 1（ERR-EQ1LUY 同族）。
      {
        const dkeyU = `${exec?.name}:${JSON.stringify(exec?.arguments || {})}`;
        if (!state.deniedKeys) state.deniedKeys = new Set();
        state.deniedKeys.add(dkeyU);
        if (state.deniedKeys.size > 500) {
          const oldestU = state.deniedKeys.keys().next().value;
          if (oldestU !== undefined) state.deniedKeys.delete(oldestU);
        }
      }
      return {
        kind: "deny",
  ...
  // 根因 = 扣减（本钩子）先于 guard 层的 decideContractAction（该处用已扣 contract 重判 → 2+1>2 → 误拒）。
  ...
  ctx.on("tools/pre-execute", async (exec, next) => {
    try {
      ...
      if (dec.outcome === "deny") {
        ...
        // 件 B（2026-09-22）：契约 deny 补卡片数据源（形状同 L1904）——只 audit 不写卡片会让回合卡片
        // verdict=clear／「本回合无裁决事件」，用户看不到这次拦截（32de10d3 实例 1–6 即此）。
        try {
          const sCard = getSessionState(state, sid);
          (sCard.turn.cardHits ||= []).push({
            tool: exec?.name || "",
            args: summarizeArgs(exec?.arguments || {}),
            ruleId: "__task-contract",
            title: `任务契约拒绝：${dec.reasonCode}`,
            reason: dec.reason,
            errId: ""
          });
          state.lastActive = [{ ruleId: "__task-contract", title: `任务契约拒绝：${dec.reasonCode}`, reason: dec.reason }];
        } catch { /* 卡片是展示层增强，失败不影响拦截本身 */ }
        // 件 C（2026-09-22）：契约 deny 同样记入 deniedKeys（钥匙算法与 L852／L1915 逐字同，含 500 上限）
        {
          const dkeyC = `${exec?.name}:${JSON.stringify(exec?.arguments || {})}`;
          if (!state.deniedKeys) state.deniedKeys = new Set();
          state.deniedKeys.add(dkeyC);
          if (state.deniedKeys.size > 500) {
            const oldestC = state.deniedKeys.keys().next().value;
            if (oldestC !== undefined) state.deniedKeys.delete(oldestC);
          }
        }
        // 执行单 3-4 已选 (b)（2026-09-22）：契约 deny 同样记下被拦那次调用的操作类型（any 不写）
        {
          const opD = operationOf(exec?.name, exec?.arguments || {});
          if (opD && opD.type && opD.type !== "any") {
            s.lastDeniedType = opD.type;
  ...
  ctx.on("tools/pre-execute", async (exec, next) => {
    try {
      ...
      // C2-C 档（批 3，2026-09-15）：bypass 窗口内让步（语义＝全部守卫暂停，与 guard status 文案一致）；
      // 窗口内的变更类调用由 guard 路径既有的 bypass-action 审计统一留痕（M2），此处不重复记。
      const freeze = criticismFreezeDecision(s, exec?.name, exec?.arguments, { bypassActive: state.bypassUntil > Date.now() });
      ...
      // 卡片数据源补齐（2026-09-11 P7）：guard 路径（本文件的 guard 命中分支）在命中时写 cardHits，
      // 本闸此前只 audit → 卡片被记成「本回合无裁决事件」（verdict=clear），用户看不到这次拦截
      // —— 与 turn-card.js 注释声明的「全量记，含 __ 内部拦截」不符，属遗漏。
      try {
        (s.turn.cardHits ||= []).push({
          tool: exec?.name || "",
          args: summarizeArgs(exec?.arguments || {}),
          ruleId: "22",
          title: "批评回合冻结写类工具",
