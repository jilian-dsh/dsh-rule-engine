...
export function handleSessionEvent(ctx, session, event) {
  ...
  if (event.type === "turn/end") {
    ...
    // P2：m8.enabled=false 可关闭（默认开启=现状行为）；entryMarker 可配置
    const m8Cfg = state.localIntegrations?.m8 || {};
    const m8Marker = m8Cfg.entryMarker || "example-manual-write.mjs";
    if (m8Cfg.enabled !== false && s.turn.manualWriteSeen && !s.turn.engramStoreSeen) {
      audit({
        kind: "engram-gap",
        rule: "__engram-gap",
        name: "双通道记忆缺失",
  ...
  if (event.type === "tool/result") {
    ...
    // P2：entryMarker 可配置（localIntegrations.m8.entryMarker）
    if (!isError && pendingCall) {
      const cmd = pendingCall.args?.command || pendingCall.args?.code || "";
      const m8Cfg2 = state.localIntegrations?.m8 || {};
      const marker = m8Cfg2.entryMarker || "example-manual-write.mjs";
      if ((pendingCall.name === "pwsh" || pendingCall.name === "bash") && cmd.includes(marker)) {
