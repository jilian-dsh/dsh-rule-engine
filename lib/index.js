...
function handleSessionEvent(ctx, session, event) {
  ...
  if (event.type === "tool/result") {
    ...
    // 规则 27：装配变更成功后置 dirty；审计命令输出通过/失败后更新状态
    if (pendingCall) {
      if (!isError && isAssemblyMutationTool(pendingCall.name, pendingCall.args)) {
        s.mountDirty = true;
        s.mountAuditPassed = false;
        audit({
          kind: "mount-dirty",
          rule: "27",
          name: "插件装配变更",
          event: "tool/result",
          reason: "装配已变更，继续装配/重启前需先跑全量审计",
          session: sid,
      ...
      if (isAuditCommand(auditCmd)) {
        ...
        if (isError || auditOutputFailed(output)) {
          ...
          s.mountDirty = false;
          s.mountAuditPassed = true;
          audit({
            kind: "mount-audit-pass",
            rule: "27",
            name: "全量审计通过",
            event: "tool/result",
            reason: "audit-mount-consistency 输出 MOUNT CONSISTENT",
            session: sid,
            tool: pendingCall.name
          });
        }
    ...
    if (!isError && pendingCall && pendingCall.originalContent != null && versionTarget && isVersionedFile(versionTarget)) {
      try {
        ...
        if (!check.ok) {
          ...
          const reason = `已回滚：${check.errors.join("；")}`;
          audit({
            kind: "rollback",
    ...
    if (pending) {
      ...
      if (askResultApproved(result)) {
        ...
        if (!pathPrefix) authRecord.expiresAt = Date.now() + 2 * 60 * 1000;
  ...
  if (event.type === "assistant/message") {
    ...
    const violations = detectViolations({ configs: state.configs, session: s, text, reasoningText: s.turn.reasoningText });
