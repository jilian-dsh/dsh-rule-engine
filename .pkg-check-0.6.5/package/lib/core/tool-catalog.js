...
const ANALYSIS_TOOLS = new Set([
  ...
  "report",
  // 官方交付物声明工具（2026-09-14 补录：verify-all ⑤「工具箱覆盖」实测 COVERAGE-FAIL —— 官方
  // tool-catalog 全集含 present 而本表未收录，会被 unknownToolDecision 当未知工具首调处置）：
  // present 只把**已存在**的文件登记为交付物（纯会话态；不复制内容、不写盘、不改文件）→ analysis。
  // 与相邻两类工具的分界：report/visualize 会写 artifact（visualize 落一份卡片副本），present 不写。
  "present",
