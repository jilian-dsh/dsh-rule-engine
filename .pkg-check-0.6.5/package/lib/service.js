...
// 2026-09-14 修（B0-2 实测踩坑）：readFile 必须来自 node:fs/promises——
// 从 node:fs 引的是回调版（第三参须为函数），按 Promise 写法 await readFile(p, "utf8")
// 会抛 `The "cb" argument must be of type function. Received type string ('utf8')`；
// 该异常被本文件 whitelistStatus 的 try/catch 吞进 debug.readError，外层仍回 ok:true
// → 面板把"读取失败"显示成"白名单为空"（静默弱化）。readFileSync 仍走 node:fs。
...
class RuleEngineService extends TypertRemoteService {
  ...
  /** 工具放行白名单状态（B0-2，2026-09-14 自 dsh-rules-manager 移植）：
   *  永久清单（`rule-engine-tools.json`）+ 近 24h 会话放行记录（从 `rule-engine.log.jsonl` 派生）；
   *  `debug` 字段供面板白名单区异常时一眼定位。移植理由＝引擎自带设置页（B0）不该依赖规则管理包。 */
