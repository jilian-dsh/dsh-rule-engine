...
export const DEFAULT_MESSAGES = {
  ...
  // C3（批 3，2026-09-15，用户裁定方案 A）：强信号 + 完整执行许可 → 不冻结，但**不静默放行**（留痕 + 本条提示）。
  // 措辞须为陈述式：注入受 INJECT_COMMAND_RE 静态闸约束（不得含「请直接执行／请立即／立刻执行」等命令词）。
  "criticism.strong-permitted": "User message matched a strong criticism signal BUT also carries a complete execution permission (approval word + action speech) — write-class tools are NOT frozen this turn and that permission stands. The criticism itself still grants nothing extra.",
  ...
  // ── D（2026-09-13）：提示前移到落盘那一刻（陈述式，不含命令词；区别于上面的 turn/end 违规判定文案）──
  "memory.sediment-hint": "This turn persisted manual/AGENTS changes through the configured entry script; the memory-sediment chain expects an engram_store call in this same turn.",
