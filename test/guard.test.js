...
hit = guardDecision(state, { name: "write", arguments: { file_path: "D:/example workspace/dsh-project/rule-guard-dev/dsh-rule-engine/test/中文.ps1", content: "Write-Output '中文'" } });
assert.ok(hit && hit.ruleId === "9", "deny Chinese ps1 write");
hit = guardDecision(state, { name: "write", arguments: { file_path: "D:/example workspace/dsh-project/rule-guard-dev/dsh-rule-engine/test/ok.ps1", content: "Write-Output 'ascii'" } });
...
markBackupSeen(stateQuoted, "global", "D:/example workspace/dsh-project/_session-repair/session-a.jsonl.zstd");
markAskSeen(stateQuoted, "global");
hit = guardDecision(stateQuoted, {
  name: "pwsh",
  arguments: { command: "Remove-Item -LiteralPath 'D:\\example workspace\\dsh-project\\_session-repair\\session-a.jsonl.zstd'" }
});
...
const exec = { name: "edit", arguments: { file_path: "D:/example workspace/dsh-project/rule-guard-dev/dsh-rule-engine/test/x.txt", old_string: "a", new_string: "b" } };
